...
Refresh token support, reservation/order/payment/ticket ownership, QR generation, ticket validation, SMTP notification email delivery, admin notification endpoints, request correlation logging, correlation id propagation into outbox/Kafka, Kafka-backed MongoDB audit logging, and ELK local log search exist. The next practical follow-ups are:
